...
function main() 
{
    ...
    let cuadrado = createCuadrado(gl);
    let triangulo = createTriangulo(gl);
    ...
    mat4.translate(modelViewMatrix, modelViewMatrix, [-1.0, 2.0, -3.333]);

    bindShaderAttributes(gl, shaderProgram);
    draw(gl, shaderProgram, cuadrado);
  

    mat4.identity(modelViewMatrix);
    
    mat4.translate(modelViewMatrix, modelViewMatrix, [1, 2.0, -3.333]);

    bindShaderAttributes(gl, shaderProgram);
    draw(gl, shaderProgram, triangulo);
...
function initGL(gl, canvas)
{
    ...
    mat4.perspective(projectionMatrix, Math.PI / 2, canvas.width / canvas.height, 1, 10000);
}
...
function createCuadrado(gl) 
{
    ...
    let cuadrado = {buffer:vertexBuffer, vertSize:3, nVerts:4, primtype:gl.TRIANGLE_STRIP};

    return cuadrado;
}

function createTriangulo(gl)
{
    let vertexBuffer;
    vertexBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
    ...
    let triangulo = {buffer:vertexBuffer, vertSize:3, nVerts:3, primtype:gl.TRIANGLES};
    return triangulo;
}

const createRombo = (gl) => {
...
const createPacMan = (gl) => {
    ...
    let startY = 0.0;
    let rad = 0.5
    verts.push(startX, startY, 0.0);

   
    for(let i = 5; i <= vertexCount - 5; i++){
        const angle = (2.0 * Math.PI * i) / vertexCount;

        const x = startX + rad * Math.cos(angle);
        const y = startY + rad * Math.sin(angle);
